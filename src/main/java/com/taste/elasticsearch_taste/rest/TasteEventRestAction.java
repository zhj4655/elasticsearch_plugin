...
import com.taste.elasticsearch_taste.grpc.ConnClient;
import com.taste.elasticsearch_taste.grpc.FormatGrpcData;
...
public class TasteEventRestAction extends BaseRestHandler{
    ...
    protected RestChannelConsumer prepareRequest(RestRequest request, NodeClient client) throws IOException {
        ConnClient grpcClient = new ConnClient("localhost", 12345);
        ...
        /**调用grpc传输/接收数据 */
        if(action != null){
            List<ConnRequest> requestList = new ArrayList<>();
            requestList.add(FormatGrpcData.formatOneData(action));
            try {
                List<String> responseList = grpcClient.sendRequest(requestList);
